...
df_train = pd.read_csv('spiral_train_data.csv', index_col=None,
                       header=None)

df_label = pd.read_csv('spiral_train_label.csv', index_col=None,
                       header=None)

df_test = pd.read_csv('spiral_test_data.csv', index_col=None,
                      header=None)

df_test_label = pd.read_csv('spiral_test_label.csv', index_col=None,
                            header=None)


x_train = df_train.to_numpy().T
y_train = df_label.to_numpy().T
x_test = df_test.to_numpy().T
y_test = df_test_label.to_numpy().T
print(x_train.shape)
print(y_train.shape)
print(x_test.shape)
...
parameters = nn_model(x_train, y_train, n_h=8,
                      num_iterations=10000, print_cost=True)

predictions = predict(parameters, x_test)

pred = []
for x in np.nditer(predictions, flags=['external_loop', 'buffered'], order='F'):
...
with open("test_predictions.csv", "w") as file1:
    # Writing data to a file
    for line in pred:
        file1.write(f"{line}\n")

# with open("spiral_test_label.csv", "r+") as file1:
#     # Reading form a file
#     file_r = [line.rstrip() for line in file1.readlines()]
#     city_list = []
